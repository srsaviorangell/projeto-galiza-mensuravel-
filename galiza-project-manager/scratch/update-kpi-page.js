...
// 1. Destructuring AppContext and state replacement
const oldStateBlock = `export default function KPIs() {
...
const newStateBlock = `export default function KPIs() {
  ...
  const modalChartData = useMemo(() => {
    ...
    const byDate: Record<string, { total: number, count: number, timestamp: number }> = {};
    (kpiCollections || []).forEach((c: any) => {
      ...
      if (selectedMetric === 'count') {
        ...
        if (c.valores?.Hora_abertura_OS && c.valores?.Hora_diagnóstico_confirmado) {
        ...
        itemVal = Number(c.valores?.[pname] || 0);
      }
  ...
  const associatedTasks = useMemo(() => {
    if (!selectedKPI) return [];
    return tasks.filter((t: any) => t.kpiEnabled && t.kpiCode === selectedKPI.code);
  }, [selectedKPI, tasks]);
...
// 2. kpiData block replacement
const oldKpiDataBlock = `  const kpiData = useMemo(() => {
    const now = new Date();
...
const newKpiDataBlock = `  const kpiData = useMemo(() => {
    const now = new Date();
...
if (!content.includes(oldKpiDataBlock)) {
  console.error("kpiData block not found!");
  process.exit(1);
}
content = content.replace(oldKpiDataBlock, newKpiDataBlock);

// 3. Mini-chart generation data replacement
...
if (!content.includes(oldChartData)) {
  console.error("chartData block not found!");
  process.exit(1);
}
content = content.replace(oldChartData, newChartData);

// 4. Global Performance mini chart placeholder removal
...
// 5. selectedKPI details modal body and chart block replacement
const oldModalBody = `                <div>
...
const newModalBody = `                <div>
                  ...
                  <p style={{ 
                      ...
                      Monitoramento em tempo real dos parâmetros: <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>
                        {selectedKPI.linkedParams && selectedKPI.linkedParams.length > 0
                          ? selectedKPI.linkedParams.map((pid: string) => allParams.find(p => p.id === pid)?.name).join(' • ')
...
if (!content.includes(oldModalBody)) {
  console.error("oldModalBody block not found!");
  process.exit(1);
}
content = content.replace(oldModalBody, newModalBody);

// 6. Sub-modal task detail creation at the end of return statement
const oldEnding = `      {/* Modal Editar KPI */}
...
if (!content.includes(oldEnding)) {
  console.error("oldEnding block not found!");
  process.exit(1);
}
content = content.replace(oldEnding, newSubModalPortal);
